import React, { useState, useCallback, useRef, useEffect } from 'react';
  import { View, Text, Image, TouchableOpacity, RefreshControl, ScrollView, StyleSheet } from '@tarojs/components';
  import Taro from '@tarojs/taro';
  import { useFocusEffect } from '@tarojs/taro';
...
const AvatarMenu = () => {
  ...
  return (
    ...
    >
      ...
      {userInfo.id ? (
        <>
          <Menu.Item title="逛一逛" leadingIcon="home-outline" onPress={() => { Taro.navigateTo({ url: '/pages/首页/首页' }); closeMenu(); }} />
          <Divider />
          <Menu.Item title="修改信息" leadingIcon="account-details-outline" onPress={() => { Taro.navigateTo({ url: '/pages/修改用户信息/修改用户信息' }); closeMenu(); }} />
          <Divider />
      ...
      {collectedTravels.length === 0 ?
        ...
        <ScrollView
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {collectedTravels.map((item, index) => (
              <View
                key={item._id}
                style={{
                  width: '50%',
          ...
          <View style={{ paddingBottom: 10, alignSelf: 'center' }}><Text style={{ fontSize: 14 }}>没有更多内容了~</Text></View>
        </ScrollView>
}
...
export default function MyTravelsScreen() {
  ...
  const fetchTravels = async () => {
    try {
      ...
      const response1 = await Taro.request({
    header: { 'Content-Type': 'application/json', 'token': token },
        url: `${NGROK_URL}/travels/getMyTravels`,
        method: 'GET',
        header: { 'token': token }
      });
      const response2 = await Taro.request({
        url: `${NGROK_URL}/travels/getCollectedTravels`,
        method: 'GET',
        header: { 'token': token }
      });
      const response3 = await Taro.request({
        url: `${NGROK_URL}/travels/getlikedTravels`,
        method: 'GET',
        header: { 'token': token }
      });
      const response4 = await Taro.request({
        url: `${NGROK_URL}/travels/getDraftTravels`,
        method: 'GET',
        header: { 'token': token }
      });
      ...
      if (response2.data.result) {
        const formattedCollectedData = response2.data.result.map(travel => {
          ...
          return {
            ...
            height: Math.floor(firstPhoto.height / firstPhoto.width * Math.floor(windowWidth / 2)),
