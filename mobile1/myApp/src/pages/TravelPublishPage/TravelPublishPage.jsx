  ...
  const pickImage = async () => {  // 选取图片，异步操作
    ...
    if (tempFilePaths.length > 0) {
      for (let i = 0; i < tempFilePaths.length; i++) {
        const uri = tempFilePaths[i];
        const { name } = tempFiles[i];
        const { width, height } = tempFiles[i];
        uri_image.push(uri);
        myform.push({
          uri,
          name,
  ...
  const onSubmit = async (data) => {
    if (file.length > 0) {  // 如果文件存在
      ...
      await Taro.uploadFile({
        url: NGROK_URL + '/travels/upload',
        filePath: file.map(f => f.uri),
        name: 'file',
        formData: params
      }).then(
        res => {
      ...
      Taro.showModal({ title: '提示', content: "您还没有上传图片，请上传图片后再发布" });
    }
  };
  ...
  const onDraftSubmit = async (data) => {
    if (file.length > 0) {
      ...
      await Taro.uploadFile({
        ...
        res => {
          console.log(res.data);
          Taro.showModal({ title: '提示', content: "草稿保存成功" });
          setIsLoading(false);
        }
      ).catch(
        err => {
